...
# in paper, training data is taken only from 2006 - 2015, here they take it up to 2016, but will exclude it out later
train_time_scale = slice('2006', '2016')
val_time_scale = slice('2016', '2016')
test_time_scale = slice('2017', '2018')

paths_to_data = [str(cwd) + '/data/geopotential_500_5.625deg/*.nc', str(cwd) + '/data/temperature_850_5.625deg/*.nc', str(cwd) +
                 '/data/2m_temperature_5.625deg/*.nc', str(cwd) + '/data/10m_u_component_of_wind_5.625deg/*.nc', str(cwd) + '/data/10m_v_component_of_wind_5.625deg/*.nc']
const_info_path = [str(cwd) + '/data/constants_5.625deg.nc']
levels = ["z", "t", "t2m", "u10", "v10"]
paths_to_data = paths_to_data[0:5]
levels = levels[0:5]

assert len(paths_to_data) == len(
    levels), "Paths to different type of data must be same as number of types of observations"
...
for idx, data in enumerate(paths_to_data):
    # mean = max of that quantity in time range(2006-2018) | std = min of that quantity in time range (2006-2018)
    Train_data, Val_data, Test_data, time_steps, lat, lon, mean, std, time_stamp = get_train_test_data_without_scales_batched(
        data, train_time_scale, val_time_scale, test_time_scale, levels[idx], args.spectral)
    max_lev.append(mean)
    min_lev.append(std)
    ...
    if idx == 0:
        ...
        # concatenate at singleton dimension
...
print("Shape of training data", Final_train_data.shape)
print("Shape of validation data", Final_val_data.shape)
print("Shape of testing data", Final_test_data.shape)
...
model = Climate_encoder_free_uncertain(len(paths_to_data), 2, out_types=len(
    paths_to_data), method=args.solver, use_att=True, use_err=True, use_pos=False).to(device)
...
# breakpoint()
fit_velocity(time_idx, time_loader, Final_train_data, Train_loader, torch.device('cpu'), num_years, paths_to_data,
             args.scale, H, W, types='train_10year_2day_mm', vel_model=Optim_velocity, kernel=kernel, lat=lat, lon=lon)
fit_velocity(time_idx, time_loader, Final_val_data, Val_loader, torch.device('cpu'), 1, paths_to_data,
             args.scale, H, W, types='val_10year_2day_mm', vel_model=Optim_velocity, kernel=kernel, lat=lat, lon=lon)
fit_velocity(time_idx, time_loader, Final_test_data, Test_loader, torch.device('cpu'), 2, paths_to_data,
             args.scale, H, W, types='test_10year_2day_mm', vel_model=Optim_velocity, kernel=kernel, lat=lat, lon=lon)

vel_train, vel_val = load_velocity(
    ['train_10year_2day_mm', 'val_10year_2day_mm'])
...
for epoch in range(args.niters):
    ...
    for entry, (time_steps, batch) in enumerate(zip(time_loader, Train_loader)):
        ...
        data = batch[0].to(device).view(
            num_years, 1, len(paths_to_data)*(args.scale+1), H, W)
        past_sample = vel_train[entry].view(
            num_years, 2*len(paths_to_data)*(args.scale+1), H, W).to(device)
        model.update_param([past_sample, const_channels_info.to(
            device), lat_map.to(device), lon_map.to(device)])
    ...
    for entry, (time_steps, batch) in enumerate(zip(time_loader, Val_loader)):
        data = batch[0].to(device).view(
            1, 1, len(paths_to_data)*(args.scale+1), H, W)
        past_sample = vel_val[entry].view(
            1, 2*len(paths_to_data)*(args.scale+1), H, W).to(device)
        model.update_param([past_sample, const_channels_info.to(
            device), lat_map.to(device), lon_map.to(device)])
        t = time_steps.float().to(device).flatten()
        mean, std, _ = model(t, data)
    ...
    if val_loss < best_loss:
        best_loss = val_loss
        best_epoch = epoch
        torch.save(model, str(cwd) + "/Models/" + "ClimODE_global_" +
                   args.solver+"_"+str(args.spectral)+"_model_" + str(epoch) + ".pt")
