...
class ResidualBlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        # reflection padding for top and bottm and circular padding for left and right
        x_mod = F.pad(F.pad(x, (0, 0, 1, 1), 'reflect'),
                      (1, 1, 0, 0), 'circular')
...
class Self_attn_conv_reg(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x.float()
        q, k, v = self.query(
            x).flatten(-2, -1), self.key(x).flatten(-2, -1), self.value(x).flatten(-2, -1)
        beta = F.softmax(torch.bmm(q.transpose(1, 2), k), dim=1)
        o = torch.bmm(v, beta.transpose(1, 2))
        o = self.post_map(
            o.view(-1, self.out_ch, size[-2], size[-1]).contiguous())
        return o
