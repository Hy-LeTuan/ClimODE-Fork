...
class Optim_velocity(nn.Module):
    ...
    def forward(self, data):
        # compute gradient of u with respect to 2 dimensions x and y
...
class Climate_ResNet_2D(nn.Module):
    ...
    def __init__(self, num_channels, layers, hidden_size):
        ...
        for idx in range(len(layers)):
            if idx == 0:
                layers_cnn.append(self.make_layer(
...
class Climate_encoder_free_uncertain(nn.Module):
    ...
    def __init__(self, num_channels, const_channels, out_types, method, use_att, use_err, use_pos):
        ...
        self.vel_f = Climate_ResNet_2D(
            input_channels, self.layers, self.hidden)
    ...
    def pde(self, t, vs):
        ...
        ds = vs[:, -self.out_ch:, :,
                :].view(-1, self.out_ch, vs.shape[2], vs.shape[3]).float()
        v = vs[:, :2*self.out_ch, :,
               :].view(-1, 2*self.out_ch, vs.shape[2], vs.shape[3]).float()
        t_emb = ((t*100) % 24).view(1, 1, 1,
                                    1).expand(ds.shape[0], 1, ds.shape[2], ds.shape[3])
        ...
        if self.pos:
            ...
            cos_lat_map, sin_lat_map = torch.cos(
                self.new_lat_map), torch.sin(self.new_lat_map)
            cos_lon_map, sin_lon_map = torch.cos(
                self.new_lon_map), torch.sin(self.new_lon_map)
            t_cyc_emb = torch.cat([day_emb, seas_emb], dim=1)
            pos_feats = torch.cat([cos_lat_map, cos_lon_map, sin_lat_map, sin_lon_map,
                                  sin_lat_map*cos_lon_map, sin_lat_map*sin_lon_map], dim=1)
            pos_time_ft = self.get_time_pos_embedding(t_cyc_emb, pos_feats)
            comb_rep = torch.cat([t_emb/24, day_emb, seas_emb, nabla_u, v, ds, self.new_lat_map,
                                 self.new_lon_map, self.lsm, self.oro, pos_feats, pos_time_ft], dim=1)
        ...
        adv2 = ds*(torch.gradient(v_x, dim=3)
                   [0] + torch.gradient(v_y, dim=2)[0])
    ...
    def noise_net_contrib(self, t, pos_enc, s_final, noise_net, H, W):
        ...
        sin_t_emb = torch.sin(torch.pi*t_emb/12 - torch.pi /
                              2).expand(len(s_final), s_final.shape[1], 1, H, W)
        cos_t_emb = torch.cos(torch.pi*t_emb/12 - torch.pi /
                              2).expand(len(s_final), s_final.shape[1], 1, H, W)

        sin_seas_emb = torch.sin(
            torch.pi*t_emb/(12*365) - torch.pi/2).expand(len(s_final), s_final.shape[1], 1, H, W)
        cos_seas_emb = torch.cos(
            torch.pi*t_emb/(12*365) - torch.pi/2).expand(len(s_final), s_final.shape[1], 1, H, W)

        pos_enc = pos_enc.expand(
            len(s_final), s_final.shape[1], -1, H, W).flatten(start_dim=0, end_dim=1)
        t_cyc_emb = torch.cat([sin_t_emb, cos_t_emb, sin_seas_emb,
                              cos_seas_emb], dim=2).flatten(start_dim=0, end_dim=1)

        pos_time_ft = self.get_time_pos_embedding(t_cyc_emb, pos_enc[:, 2:-2])

        comb_rep = torch.cat([t_cyc_emb, s_final.flatten(
            start_dim=0, end_dim=1), pos_enc, pos_time_ft], dim=1)
    ...
    def forward(self, T, data, atol=0.1, rtol=0.1):
        ...
        if self.pos:
            ...
            self.lsm = self.lsm.unsqueeze(dim=0).expand(
                data.shape[0], -1, data.shape[3], data.shape[4])
            self.oro = F.normalize(self.const_info[0, 0]).unsqueeze(
                dim=0).expand(data.shape[0], -1, data.shape[3], data.shape[4])
            self.new_lat_map = self.lat_map.expand(
                data.shape[0], 1, data.shape[3], data.shape[4])*torch.pi/180  # Converting to radians
            self.new_lon_map = self.lon_map.expand(
                data.shape[0], 1, data.shape[3], data.shape[4])*torch.pi/180
            cos_lat_map, sin_lat_map = torch.cos(
                self.new_lat_map), torch.sin(self.new_lat_map)
            cos_lon_map, sin_lon_map = torch.cos(
                self.new_lon_map), torch.sin(self.new_lon_map)
        ...
        new_time_steps = torch.linspace(
            init_time, final_time, steps=int(steps_val)+1).to(data.device)
        t = 0.01*new_time_steps.float().to(data.device).flatten().float()
        def pde_rhs(t, vs): return self.pde(
            t, vs)  # make the ODE forward function
        final_result = odeint(pde_rhs, final_data, t,
                              method=self.method, atol=atol, rtol=rtol)
        # breakpoint()
        s_final = final_result[:, :, -self.out_ch:,
                               :, :].view(len(t), -1, self.out_ch, H, W)
...
class ClimODE_encoder_free_uncertain(nn.Module):
    ...
    def pde(self, t, vs):
        ...
        ds = vs[:, -self.out_ch:, :, :].float().view(-1, self.out_ch,
                                                     vs.shape[2], vs.shape[3]).float()
        v = vs[:, :2*self.out_ch, :, :].float().view(-1, 2*self.out_ch,
                                                     vs.shape[2], vs.shape[3]).float()
        t_emb = ((t*100) % 24).view(1, 1, 1,
                                    1).expand(ds.shape[0], 1, ds.shape[2], ds.shape[3])
        sin_t_emb = torch.sin(torch.pi*t_emb/12 - torch.pi/2)
        ...
        if self.att:
            ...
            dv = self.vel_f(comb_rep)
        v_x = v[:, :self.out_ch, :, :].float().view(-1, self.out_ch,
                                                    vs.shape[2], vs.shape[3]).float()
        v_y = v[:, -self.out_ch:, :, :].float().view(-1, self.out_ch,
                                                     vs.shape[2], vs.shape[3]).float()
...
class Climate_encoder_free_uncertain_monthly(nn.Module):
    ...
    def pde(self, t, vs):
        ...
        if self.pos:
            ...
            pos_feats = torch.cat([cos_lat_map, cos_lon_map, sin_lat_map, sin_lon_map,
                                  sin_lat_map*cos_lon_map, sin_lat_map*sin_lon_map], dim=1)
            pos_time_ft = self.get_time_pos_embedding(t_cyc_emb, pos_feats)
            comb_rep = torch.cat([t_emb/6, day_emb, nabla_u, v, ds, self.new_lat_map,
                                 self.new_lon_map, self.lsm, self.oro, pos_feats, pos_time_ft], dim=1)
    ...
    def noise_net_contrib(self, t, pos_enc, s_final, noise_net, H, W):
        ...
        sin_t_emb = torch.sin(torch.pi*t_emb/(12*6) - torch.pi /
                              2).expand(len(s_final), s_final.shape[1], 1, H, W)
        cos_t_emb = torch.cos(torch.pi*t_emb/(12*6) - torch.pi /
                              2).expand(len(s_final), s_final.shape[1], 1, H, W)
